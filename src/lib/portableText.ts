...
const portableTextComponents = {
  ...
  list: {
    bullet: ({ children }: { children: string }) => 
      `<ul class="list-disc list-inside mb-4 space-y-2 text-gray-700 dark:text-gray-200">${children}</ul>`,
    number: ({ children }: { children: string }) => 
      `<ol class="list-decimal list-inside mb-4 space-y-2 text-gray-700 dark:text-gray-200">${children}</ol>`,
  },
  listItem: {
